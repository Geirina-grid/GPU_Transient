...
operator()(const d_vector_type &x, d_vector_type &dxdt, const value_type t) {
  ...
  thrust::device_vector<double> d_Vm(GEN_SIZE);
  thrust::device_vector<double> d_Vx(GEN_SIZE);
  thrust::device_vector<double> d_Vy(GEN_SIZE);
  thrust::device_vector<double> d_Va(GEN_SIZE);
  thrust::device_vector<double> d_Vd(GEN_SIZE);
  thrust::device_vector<double> d_Vq(GEN_SIZE);
  thrust::device_vector<double> d_Ifd(GEN_SIZE);
  thrust::device_vector<double> d_x_omega_idx(GEN_SIZE);
  thrust::device_vector<double> d_x_delta_id(GEN_SIZE);
  thrust::device_vector<double> d_x_Eqp_idx(GEN_SIZE);
  thrust::device_vector<double> d_gen_a(GEN_SIZE);
  thrust::device_vector<double> d_gen_b(GEN_SIZE);
  thrust::device_vector<double> d_gen_n(GEN_SIZE);
  thrust::device_vector<double> d_gen_Ra(GEN_SIZE);
  thrust::device_vector<double> d_dxdt_Eqp_idx(GEN_SIZE);
  thrust::device_vector<double> d_gen_Xdp(GEN_SIZE);
  thrust::device_vector<double> d_gen_Xqp(GEN_SIZE);
  thrust::device_vector<double> d_x_Edp_idx(GEN_SIZE);
  thrust::device_vector<double> d_dIfd_dt(GEN_SIZE);
  thrust::device_vector<double> d_Id(GEN_SIZE);
  thrust::device_vector<double> d_Iq(GEN_SIZE);
  thrust::device_vector<double> d_Telec(GEN_SIZE);
  thrust::device_vector<int> d_gen_bus_id(GEN_SIZE);
  thrust::device_vector<double> d_Vm_ref(GEN_SIZE);
  thrust::device_vector<double> d_parameters_Vt_ref(GEN_SIZE);
  thrust::device_vector<double> d_omega_ref(GEN_SIZE);
  thrust::device_vector<double> d_parameters_omega_ref(GEN_SIZE);
  thrust::device_vector<double> d_t(GEN_SIZE);
  EPRI_GEN_DATA& gen = d_parameters.gen;

  //For generators
  thrust::device_vector<double> d_dxdt_omega_idx(GEN_SIZE);
  thrust::device_vector<double> d_TJ(GEN_SIZE);
  thrust::device_vector<double> d_EPS(GEN_SIZE);
  thrust::device_vector<double> d_Pmech(GEN_SIZE);
  thrust::device_vector<double> d_D(GEN_SIZE);
  thrust::device_vector<double> d_dxdt_delta_idx(GEN_SIZE);
  thrust::device_vector<double> d_freq_ref(GEN_SIZE);

  //Initialization
  //std::clock_t start = std::clock();
  for(int i = 0; i < GEN_SIZE; i++){
      //3D Host paramater to device
      d_x_omega_idx[i] = x[i*GEN_SIZE + 0];
      d_x_delta_id[i] = x[i*GEN_SIZE + 1];
      d_x_Eqp_idx[i] = x[i*GEN_SIZE + 2];
      d_dxdt_Eqp_idx[i] = dxdt[i*GEN_SIZE + 2];
      d_gen_a[i] = gen.a;
      d_gen_b[i] = gen.b;
      d_gen_n[i] = gen.n;
      d_gen_Ra[i] = gen.Ra;
