...
typedef thrust::device_vector< value_type > d_state_type;
...
struct lorenz_system
{
    ...
    lorenz_system( size_t N , const d_state_type &beta )
    : m_N( N ) , m_beta( beta ) { }
...
struct lorenz_perturbation_system
{
    ...
    lorenz_perturbation_system( size_t N , const d_state_type &beta )
    : m_N( N ) , m_beta( beta ) { }
...
extern "C" void test_cuda(map<string, GENERATOR> generators)
{
    ...
    typedef runge_kutta_dopri5< d_state_type , value_type , d_state_type , value_type > stepper_type;
    ...
    for (auto &g_hldr : generators) {
        ...
        //uint__t bus_id = Transient_Solver::bus_name_to_id[bus_name] -1 ;
    }
